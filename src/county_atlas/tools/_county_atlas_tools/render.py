...
def build_atlas_html(catalog: dict, materialized: dict, counts: dict,
    ...
    svg_body = "".join(g for _o, g in sorted(groups, key=lambda x: x[0]))
    ...
    for cat in categories:
        ...
        for lyr in layers:
            if lyr["category"] != cat:
                continue
            lid = lyr["id"]
            live = lid in materialized and counts.get(lid, 0) > 0
    ...
    n_live = sum(1 for l in layers if l["id"] in materialized and counts.get(l["id"], 0) > 0)
...
<script>document.querySelectorAll('input[data-layer]').forEach(function(cb){{cb.addEventListener('change',function(){{var g=document.querySelector('g[data-layer="'+cb.dataset.layer+'"]');if(g)g.style.display=cb.checked?'':'none';}});}});</script>
</div>"""

...
| **Tier-2 EPA join** — TRI (per-county) + Superfund/Brownfields (shared national cache) → points | [`epa.py`](src/county_atlas/tools/_county_atlas_tools/epa.py) |
...
| **Tier-2 HUD join** — public-housing points | [`hud.py`](src/county_atlas/tools/_county_atlas_tools/hud.py) |
| **TIGER boundary overlays** — block groups + school districts | [`tiger.py`](src/county_atlas/tools/_county_atlas_tools/tiger.py) |
...
| `workflows.BuildAtlasFanout(prefix, tier, bucket)` | **Fan-out** — one `BuildCountyAtlas` per county, fleet-wide |
